...
// 设计图为2560情况下 html 字体大小是100px
// 那么实际宽度为xxx的情况下 字体大小是 x
2560/100 = xxx/x => 25.6 = xxx / x => x = xxx / 25.6 js的计算方法
html { font-size: calc(100vw / 3.75) } css的计算方式
...
rem的适用性很有局限，仅仅只能够用于只在移动端展示的页面。如果你的页面还需要适配到 pc 端，那么就老老实实的使用 px 吧。

----------------------------------------

1.  在用nz-menu做导航时, 可以用一个a元素当做浮层,覆盖整个li,来做路由跳转,避免只能点字体跳转的尴尬
...
发送请求的Observable，是一个发生一次就完成的Observable，所以可以用于forkjoin，这里注意forkjoin的用法，是所有的Observable[都完成]时，才发送结果的Observable
...
***重点***
***121. 有时候dom没有显示，[先检查]dom结构，多半是样式的问题
例子，arcgis的popup没有显示，也没有报错，最后发现popup的dom在文档下面，配置的arcgis的样式文件地址加了main.css?date=123
造成本地找到这个样式文件，但linux服务器找不到文件


___________________________________________________________________________________________________________

***重点***
***122. 慎用 css 的[transform], [perspective], [filter]属性
此属性可能会导致 该元素的子孙元素 把该元素作为视口，影响包括getBoundingBox() 和 其fix 定位
...
1.   使用switchMap一类的内部订阅操作符的错误处理必须在内部的订阅里
...
this.select$.asObservable().pipe(
            ...
            this.mvs.selectedFeatures.next([graphic]);
...
const stream1$ = concat(...data.map((d, i) => 
...
1.   openlayer 的boundingExtent直接用于计算多个点所在的范围
```javascript
fit(features: any[]) {
...
1.   防止ant-table的column resize 触发sort
...
***143. [表单]
ControlValueAccessor 太坑了，[registerOnChange]和[registerOnTouched]只能实现一个，不然数据[双向绑定失效]!!!!!!
...
150.   关于a标签的下载
如果url不是base64(DataUrl)而是http协议的网页，a标签不会去下载，而是会打开网页，这个时候要想实现下载的效果只能get获取文件内容然后生成blob后，再下载
```javascript
getPTZTxtFileContent(url) {
    return this.http.get(url, {responseType: 'text', context: WithoutAuthTokenContext});
}


getFile() {
    this.eps.getPTZTxtFileContent(this.file?.url).subscribe(content => {
        const blob = new Blob([JSON.stringify(content)], { type: 'text/plain' });
        const objectUrl = URL.createObjectURL(blob);
        // console.log(objectUrl);
        this.tools.commonDownload(objectUrl, this.file?.fileName);
    });
}
```
________________________________________________________________________________________________________
...
152. 闭包形成的本质是内部函数可以访问(引用)外部函数的执行作用域(可以叫执行上下文对象或者函数的活动对象)，这是一种机制，所以
